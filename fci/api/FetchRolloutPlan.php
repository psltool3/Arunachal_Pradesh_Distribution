...
if (!isset($_POST['month']) || !isset($_POST['district'])) {
...
if (!is_string($month) || !is_string($district)) {
    die(json_encode(["error" => "Invalid parameter type"]));
}

if (!preg_match("/^[a-zA-Z]+_[0-9]{4}$/", $month)) {
    die(json_encode(["error" => "Invalid Month Format"]));
}
if (!preg_match("/^[a-zA-Z0-9\s\-_]+$/", $district)) {
    die(json_encode(["error" => "Invalid District Parameter"]));
}
...
if ($result && $result->num_rows > 0) {
	...
	$tableData["WH_Used_Baseline"] = '31';
	$tableData["FPS_Used_Baseline"] = '1,837';
	$tableData["Demand_Baseline"] = '62,606';
	$tableData["Total_QKM_Baseline"] = '44,37,321';
	$tableData["Average_Distance_Baseline"] = '70.88';
