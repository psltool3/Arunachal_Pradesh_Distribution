...
if (isset($_GET['format'])) {
    ...
    if (!isset($_GET['month']) || !isset($_GET['district'])) {
    ...
    // Input Validation
    if (!preg_match("/^[a-zA-Z]+_[0-9]{4}$/", $month)) {
        die('Error : Invalid Month Format');
    }
    // District can contain letters and spaces, maybe dots or hyphens. 
    // Allowing generous character set but preventing SQL injection via escaping later.
    // Ideally regex should be strict, but I don't know all district names. 
    // Using whitelist for alphanumeric and common chars.
    if (!preg_match("/^[a-zA-Z0-9\s\-_]+$/", $district) && $district != "all" && $district != "") {
         die('Error : Invalid District Parameter');
    }


	$parts = explode('_', $month);

	$month = mysqli_real_escape_string($con, $parts[0]);
	$year = mysqli_real_escape_string($con, $parts[1]); 
	...
	$tablename = "optimiseddata_".mysqli_real_escape_string($con, $id);
    
    // Validate table name structure to be safe
    if (!preg_match("/^optimiseddata_[0-9]+$/", $tablename)) {
