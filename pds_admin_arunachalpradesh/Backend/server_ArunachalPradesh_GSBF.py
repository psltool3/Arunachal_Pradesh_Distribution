...
def _job_db_init():
    with _job_db_connect() as conn:
        ...
        """)
        conn.execute("CREATE INDEX IF NOT EXISTS idx_jobs_client_endpoint ON jobs(client_id, endpoint, status)")
        conn.commit()

def _job_now_iso():
    return datetime.utcnow().isoformat() + "Z"

def _job_create(client_id, endpoint, message="queued"):
...
def _job_get_active_for_client(client_id, endpoint):
    with _job_db_connect() as conn:
        row = conn.execute(
...
def _job_reconcile_after_restart():
    ...
    with _job_db_connect() as conn:
        conn.execute(
            "UPDATE jobs SET status = 'failed', message = 'Server restarted. Job aborted.', updated_at = ? "
            "WHERE status IN ('queued', 'running') AND server_instance_id != ?",
...
def _run_job_in_background(job_id, form_data, endpoint, synchronous_handler_fn):
    try:
        _job_update(job_id, status="running", message="Processing started...")
        
        # Clone parameters. Force async to 0 to prevent infinite background spawning loops.
        safe_data = dict(form_data or {})
        safe_data["async"] = "0"
        safe_data["job_id"] = job_id
        
        # Setup request context matching the original HTTP parameters
        with app.test_request_context(endpoint, method='POST', data=safe_data):
            response = synchronous_handler_fn()
            
        if isinstance(response, tuple):
            response = response[0]
            
        result_text = response.get_data(as_text=True) if hasattr(response, "get_data") else str(response)
        
        # Handle custom internal failures that return status: 0 inside a 200 OK HTTP block
        try:
        ...
        _job_update(job_id, status="failed", message="Critical failure", error=traceback.format_exc())
...
def job_result(job_id):
    ...
    if job["status"] != "completed":
        return jsonify({"status": "error", "message": "Job not completed"}), 400
    return job["result_json"], 200, {'Content-Type': 'application/json'}
...
def active_job():
    ...
    if not client_id or not endpoint:
        return jsonify({"has_active": False, "message": "Parameters missing"}), 400
        
    job = _job_get_active_for_client(client_id, endpoint)
    if job:
        # Calculate running elapsed time on the server side
        created_time = datetime.fromisoformat(job["created_at"].rstrip("Z"))
        elapsed_seconds = int((datetime.utcnow() - created_time).total_seconds())
        return jsonify({
            "has_active": True,
            "job_id": job["job_id"],
            "status": job["status"],
...
def extract_db():
    if request.method == 'POST':
        ...
        fps = fps.loc[:,["State Name","FPS_District",'FPS_Name',"FPS_ID","Motorable/Non-Motorable",'FPS_Lat',"FPS_Long","Allocation_FRice","FPS_Tehsil"]]
...
def processFile():
    if request.form.get("async") == "1":
        client_id = request.form.get("client_id") or "anonymous"
        job_id = _job_create(client_id, "/processFile", message="Initializing...")
        form_dict = request.form.to_dict(flat=True)
        p = multiprocessing.Process(
            target=_run_job_in_background,
            args=(job_id, form_dict, "/processFile", processFile),
            daemon=True
        )
        p.start()
        return jsonify({"status": 1, "job_id": job_id, "message": "Processing started in background"})
    ...
    #    Variables for Wheat from IG TO FPS
       ...
       # Calling CBC_CMB Solver
        ...
        df51.insert(2, 'From_State', 'Arunachal Pradesh')
        df51.insert(7, 'To', 'FPS')
        df51.insert(8, 'To_State', 'Arunachal Pradesh')
        ...
        with pd.ExcelWriter('Backend//ArunachalPradesh_L2_Distance.xlsx') as writer:
            Distance3.to_excel(writer, sheet_name='BG_BG',index=False)
            

        Cost = pd.ExcelFile("Backend//ArunachalPradesh_L2_Distance.xlsx")
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        files_to_delete = [
            'Backend/ArunachalPradesh_L2_Distance.xlsx',
...
def processFile_leg1():
    if request.form.get("async") == "1":
        client_id = request.form.get("client_id") or "anonymous"
        job_id = _job_create(client_id, "/processFileleg1", message="Initializing...")
        form_dict = request.form.to_dict(flat=True)
        p = multiprocessing.Process(
            target=_run_job_in_background,
            args=(job_id, form_dict, "/processFileleg1", processFile_leg1),
            daemon=True
        )
        p.start()
        return jsonify({"status": 1, "job_id": job_id, "message": "Processing started in background"})
    ...
    if scenario_type == "intra":
       ...
       # Calling CBC_CMB Solver
        ...
        Distance3 = Distance2.drop(columns=Distance2.columns[Distance2.columns.isin(FPS_ID)])
        Distance3 = Distance3.T
        with pd.ExcelWriter('Backend//arunachal_L1_Distance.xlsx') as writer:
            Distance3.to_excel(writer, sheet_name='BG_BG',index=False)
            Warehouse.to_excel(writer, sheet_name='Warehouse', index=False)
            FCI.to_excel(writer, sheet_name='FCI', index=False)

        Cost = pd.ExcelFile("Backend//arunachal_L1_Distance.xlsx")
        BG_BG = pd.read_excel(Cost,sheet_name="BG_BG")
...
# --------------------------------------------------------------------------------------------------------------------------------------------- 
       ...
       # Calling CBC_CMB Solver
        ...
        Distance3 = Distance2.drop(columns=Distance2.columns[Distance2.columns.isin(FPS_ID)])
        Distance3 = Distance3.T
        with pd.ExcelWriter('Backend//ArunachalPradesh_L1_Distance.xlsx') as writer:
            Distance3.to_excel(writer, sheet_name='BG_BG',index=False)
            Warehouse.to_excel(writer, sheet_name='Warehouse', index=False)
            FCI.to_excel(writer, sheet_name='FCI', index=False)
        ...
        Cost = pd.ExcelFile("Backend//ArunachalPradesh_L1_Distance.xlsx")
        BG_BG = pd.read_excel(Cost,sheet_name="BG_BG")
        Cost.close()
        data1 = pd.ExcelFile("Backend//Tagging_Sheet_Pre11_leg1.xlsx")
        df5 = pd.read_excel(data1,sheet_name="BG_FPS1")
...
# ---------------------------------------------------------------------------------------------------------------------------------------------
        ...
        data["WH_Used"] = df5['From_ID'].nunique()
        data["WH_Used_Baseline"] = "16"
        
        data["FPS_Used"] = df5['To_ID'].nunique()
        data["FPS_Used_Baseline"] = "31"
        
        data['Demand'] = int(WH['Demand'].sum())
        data['Demand_Baseline'] = "62,606"
        
        data['Total_QKM'] = float(result)
        data['Total_QKM_Baseline'] = "22,31,829"
        
        data['Average_Distance'] = float(round(result, 2)) / Total_Demand
        data['Average_Distance_Baseline'] = "35.65"
        ...
        # List of files to delete
        files_to_delete = [
            'Backend/ArunachalPradesh_L1_Distance.xlsx',
