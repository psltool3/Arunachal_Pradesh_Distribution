...
<script>
    ...
    <!-- START SCRIPTS -->
        ...
        success:function(result){
            try{
                var districts = JSON.parse(result);
                var dropdown = document.getElementById("district");
                dropdown.innerHTML =
                "<option value=''>Select District</option><option value='all'>All</option>";
		...
		document.getElementById('downloadCSV').addEventListener('click', async function() {
			try {
				...
				downloadFile(csvBlob, 'Optimize_Plan_' + getDateString() + '.csv');
			} catch (error) {
		...
		document.getElementById('downloadPDF').addEventListener('click', async function() {
			try {
				...
				const csvBlob = await csvResponse.blob();
				downloadFile(csvBlob, 'Optimize_Plan_' + getDateString() + '.pdf');
			} catch (error) {
		...
		document.getElementById('downloadXLSX').addEventListener('click', async function() {
			try {
				...
				downloadFile(excelBlob, 'Optimize_Plan_' + getDateString() + '.xlsx');
			} catch (error) {
