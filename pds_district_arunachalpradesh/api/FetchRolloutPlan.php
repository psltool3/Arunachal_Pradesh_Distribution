...
if (!isset($_POST['status'])) {
    die(json_encode(["error" => "Missing status parameter"]));
}

$status = $_POST['status'];

if (!is_string($status)) {
    die(json_encode(["error" => "Invalid status parameter"]));
}

if ($status !== 'implemented' && $status !== 'not implemented' && $status !== 'all') { // Assuming 'all' might be default or empty
    // Basic whitelist check, though logic below handles specific values
}
$data = array();
